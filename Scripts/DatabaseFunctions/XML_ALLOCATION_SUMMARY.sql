-- Function: "WADE_R"."XML_ALLOCATION_SUMMARY"(text, text, text)

-- DROP FUNCTION "WADE_R"."XML_ALLOCATION_SUMMARY"(text, text, text);

CREATE OR REPLACE FUNCTION "WADE_R"."XML_ALLOCATION_SUMMARY"(
    IN orgid text,
    IN reportid text,
...
tmp_output:=(SELECT STRING_AGG
		(XMLELEMENT
			(name "WC:AllocationUse",
				...
				XMLELEMENT
					(name "WC:AllocationAmountSummary", XMLCONCAT
						(
							(SELECT XMLFOREST
							...
							(SELECT "WADE_R"."XML_S_ALLOCATION_IRRIGATION"(orgid, reportid, reportunitid, "SUMMARY_SEQ")
							)
						)
					)
			)::text,''
	...
	"WADE"."SUMMARY_ALLOCATION" A LEFT OUTER JOIN "WADE"."LU_BENEFICIAL_USE" B ON (A."BENEFICIAL_USE_ID"=B."LU_SEQ_NO") 
...
ELSE
	...
	tmp_output :='<WC:AllocationSummary>' || tmp_output || '</WC:AllocationSummary>';
...
END
  ...
  COST 1000;
ALTER FUNCTION "WADE_R"."XML_ALLOCATION_SUMMARY"(text, text, text)
  OWNER TO "WADE";
