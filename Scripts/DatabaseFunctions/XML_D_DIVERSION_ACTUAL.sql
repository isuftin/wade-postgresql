-- Function: "WADE_R"."XML_D_DIVERSION_ACTUAL"(text, text, text, text, numeric)

-- DROP FUNCTION "WADE_R"."XML_D_DIVERSION_ACTUAL"(text, text, text, text, numeric);

CREATE OR REPLACE FUNCTION "WADE_R"."XML_D_DIVERSION_ACTUAL"(
    IN orgid text,
    IN reportid text,
...
tmp_output := (SELECT STRING_AGG
	...
	"WADE"."D_DIVERSION_ACTUAL" A LEFT OUTER JOIN "WADE"."METHODS" B ON (A."METHOD_ID_VOLUME"=B."METHOD_ID") 
	...
	AND "DIVERSION_ID"=diversionid AND "DETAIL_SEQ_NO"=seqno);
...
END
  ...
  COST 1000;
ALTER FUNCTION "WADE_R"."XML_D_DIVERSION_ACTUAL"(text, text, text, text, numeric)
  OWNER TO "WADE";
